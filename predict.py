...
mlstm_cell = rnn.MultiRNNCell([genlstmcell() for i in range(layer_num)], state_is_tuple=True)
...
y_output = tf.argmax(y_pre, 1)


# 损失和评估函数
cross_entropy = -tf.reduce_mean(y * tf.log(y_pre))
# cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=y_pre)
...
for i in range(1):
    ...
    print("input %r, label %r" % (input_x, input_y))
    # print(input_x)
    # print(input_y.shape)
    _batch_size = 1
    output = sess.run(y_output, feed_dict={
            _X: input_x, keep_prob: 1.0, batch_size: _batch_size})
    print("test result %r" % output)

    file_object = open("testresult.txt", "w")
    file_object.write(str(output[0]))
